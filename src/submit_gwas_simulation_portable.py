...
# --------------------------------------------
# 0. HPC / environment config (edit if needed)
# --------------------------------------------
PYTHON_EXEC = os.getenv("ICEPOP_PYTHON", "python")
SLURM_ACCOUNT = os.getenv("ICEPOP_ACCOUNT", "general")
SLURM_OUTPUT_DIR = "../slurms"

Path(SLURM_OUTPUT_DIR).mkdir(exist_ok=True)
...
# --------------------------------------------
# 2. Simulation script + Slurm template
# --------------------------------------------
...
#SBATCH --output={slurm_out}/{jobname}_%j.out
