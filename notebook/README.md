...
- `preprocess_TM_FACS.ipynb`: Preprocesses Tabula Muris (TM) FACS input data (loads data, checks outlier cells, converts symbols to IDs) and saves the prepared dataset(s) for downstream ICePop analyses.
...
## Casual simulation
- `causal_sim.ipynb`: Runs the causal-simulation evaluation by comparing simulation outputs (with known ground truth signals) and comparing inferred association behavior across settings.
- `causal_sim_compare_mc.ipynb`: Compares causal-simulation performance across different ICePop metacell size (`mc-30/50/75`) configurations.
